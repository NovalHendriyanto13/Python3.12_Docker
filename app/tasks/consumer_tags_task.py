...
tablename = "consumer_tags"
past_date = date.today() -  timedelta(days=timedelta_days)

@task(retries=3, retry_delay_seconds=10, log_prints=True, name="fetch_consumer_tags")
async def fetch_data_task():
    logger = get_run_logger()
    logger.info(f"Fetching data from consumer_tags")
    ...
    rows = await run_in_threadpool(
        ...
        columns = columns,
        criteria = {
            "tag_assigned_time_utc": {"op": ">=", "value": past_date}
        }
    )
...
async def insert_to_mongo_task(rows: list):
    ...
    result = await upsert_mongo(tablename, rows, unique_key=["reporting_id", "tag_value_id", "tag_assigned_time_utc", "market"])

    logger.info(f"Inserted {result} records in collection {tablename}")
    return result

@flow(name="databricks_to_mongo_consumer_tags_sync", log_prints=True)
async def databricks_to_mongo_consumer_tags_sync():
    logger = get_run_logger()
...
if __name__ == "__main__":
    asyncio.run(databricks_to_mongo_consumer_tags_sync())
