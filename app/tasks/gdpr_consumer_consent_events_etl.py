...
async def load_chunk_to_postgres(batch: list, columns: list):
    async with engine.begin() as conn:
        ...
        await conn.execute(
            text("CREATE TEMP TABLE temp_mcd_gdpr_consumer_consent_events (LIKE mcd_gdpr_consumer_consent_events EXCLUDING ALL) ON COMMIT DROP")
        )
        
        # Stream load to staging table
        await asyncpg_conn.copy_records_to_table(
            'temp_mcd_gdpr_consumer_consent_events', records=batch, columns=columns
        )
        
        # Merge/Upsert query
        upsert_query = """
        INSERT INTO mcd_gdpr_consumer_consent_events (
            reporting_id,
            market,
...
async def extract_and_load_gdpr_consumer_consent_events_fast():
    ...
    if batch:
        await load_chunk_to_postgres(batch, columns)
        total += len(batch)
        print(f"   ✅ Synchronized chunk of {len(batch)} gdpr_consumer_consent_events (Total: {total})")
        
    print(f"🚀 Bulk Sync Finished! Total {total} gdpr_consumer_consent_events successfully upserted.")

@flow(name="mongo-to-postgres-etl", log_prints=True)
async def etl_gdpr_consumer_consent_events_flow():
    print("ETL Flow gdpr_consumer_consent_events started...")
    await extract_and_load_gdpr_consumer_consent_events_fast()
    print("ETL Flow gdpr_consumer_consent_events finished!")

if __name__ == "__main__":
    asyncio.run(etl_gdpr_consumer_consent_events_flow())