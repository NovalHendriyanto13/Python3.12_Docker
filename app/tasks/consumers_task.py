...
@task(retries=3, retry_delay_seconds=10, log_prints=True, name="fetch_consumers")
async def fetch_data_task():
    logger = get_run_logger()
    logger.info(f"Fetching data from consumers")
...
async def insert_to_mongo_task(rows: list):
    ...
    result = await upsert_mongo(tablename, rows, unique_key="reporting_id")

    logger.info(f"Inserted {result} records in collection {tablename}")
    return result

@flow(name="databricks_to_mongo_consumers_sync", log_prints=True)
async def databricks_to_mongo_consumers_sync():
    logger = get_run_logger()
...
if __name__ == "__main__":
    asyncio.run(databricks_to_mongo_consumers_sync())
