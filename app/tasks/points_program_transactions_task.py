...
tablename = "points_program_transactions"
past_date = date.today() -  timedelta(days=timedelta_days)

@task(retries=3, retry_delay_seconds=10, log_prints=True, name="fetch_points_program_transactions")
async def fetch_data_task():
    logger = get_run_logger()
    logger.info(f"Fetching data from points_program_transactions")
    ...
    rows = await run_in_threadpool(
        ...
        columns = columns,
        criteria = {
            "transaction_time_utc": {"op": ">=", "value": past_date}
        }
    )
...
@flow(name="databricks_to_mongo_points_program_transactions_sync", log_prints=True)
async def databricks_to_mongo_points_program_transactions_sync():
    logger = get_run_logger()
...
if __name__ == "__main__":
    asyncio.run(databricks_to_mongo_points_program_transactions_sync())
