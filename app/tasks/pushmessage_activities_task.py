...
tablename = "pushmessage_activities"
past_date = date.today() -  timedelta(days=timedelta_days)

@task(retries=3, retry_delay_seconds=10, log_prints=True, name="fetch_pushmessage_activities")
async def fetch_data_task():
    logger = get_run_logger()
    logger.info(f"Fetching data from pushmessage_activities")
...
@flow(name="databricks_to_mongo_pushmessage_activities_sync", log_prints=True)
async def databricks_to_mongo_pushmessage_activities_sync():
    logger = get_run_logger()
...
if __name__ == "__main__":
    asyncio.run(databricks_to_mongo_pushmessage_activities_sync())
