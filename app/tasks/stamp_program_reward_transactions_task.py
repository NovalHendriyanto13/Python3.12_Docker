...
tablename = "stamp_program_reward_transactions"
past_date = date.today() -  timedelta(days=timedelta_days)

@task(retries=3, retry_delay_seconds=10, log_prints=True, name="fetch_stamp_program_reward_transactions")
async def fetch_data_task():
    logger = get_run_logger()
    logger.info(f"Fetching data from stamp_program_reward_transactions")
...
async def insert_to_mongo_task(rows: list):
    ...
    result = await upsert_mongo(tablename, rows, unique_key="stamp_program_reward_transaction_id")

    logger.info(f"Inserted {result} records in collection {tablename}")
    return result

@flow(name="databricks_to_mongo_stamp_program_reward_transactions_sync", log_prints=True)
async def databricks_to_mongo_stamp_program_reward_transactions_sync():
    logger = get_run_logger()
...
if __name__ == "__main__":
    asyncio.run(databricks_to_mongo_stamp_program_reward_transactions_sync())
