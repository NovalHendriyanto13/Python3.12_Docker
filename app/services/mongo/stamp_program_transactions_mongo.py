...
async def upsert_stamp_program_transactions(db: AsyncSession, mongo_doc: dict):
    ...
    existing = await db.execute(
        select(StampProgramTransactions).where(
            StampProgramTransactions.activity_id == activity_id
        )
    )
    ...
    stmt=insert(StampProgramTransactions).values(**new_values).on_conflict_do_update(
    ...
    if existing_record is not None:
        StagingDataChangesService(
            db=db,
            module_name="mcd_stamp_program_transactions",
