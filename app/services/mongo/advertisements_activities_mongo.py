...
async def upsert_advertisement_activities(db: AsyncSession, mongo_doc: dict):
    ...
    existing = await db.execute(
        select(AdvertisementActivities).where(
            AdvertisementActivities.activity_id == activity_id
        )
    )
    ...
    stmt=insert(AdvertisementActivities).values(**new_values).on_conflict_do_update(
    ...
    if existing_record is not None:
        StagingDataChangesService(
            db=db,
            module_name="mcd_advertisements_activities",
