...
async def upsert_offer_activities(db: AsyncSession, mongo_doc: dict):
    ...
    existing = await db.execute(
        select(OfferActivities).where(
            OfferActivities.activity_id == activity_id
        )
    )
    ...
    stmt=insert(OfferActivities).values(**new_values).on_conflict_do_update(
    ...
    if existing_record is not None:
        StagingDataChangesService(
            db=db,
            module_name="mcd_offer_activities",
