from sqlalchemy import Column, String, SmallInteger, Date, Integer, Numeric
from configs.database import Base
from sqlalchemy.dialects.postgresql import UUID, TIMESTAMP
import uuid

class ConsumerActivities(Base):
    __tablename__ = "mcd_consumer_activities"
    
    activity_id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    action_type_code = Column(Integer)
    action_type_name = Column(String)
    action_type_detail = Column(String)
    device_type_code = Column(String)
    device_type_name = Column(String)
    market = Column(String)
    reporting_id = Column(UUID(as_uuid=True))
    date = Column(Date)
    activity_source_time_local = Column(TIMESTAMP(timezone=True))
    activity_source_time_utc = Column(TIMESTAMP(timezone=False))
    source_activity_offset = Column(String)
    hour = Column(String)
    time_of_processed = Column(TIMESTAMP(timezone=False))
    market_id = Column(String)
    download_count = Column(String)
    startup_count = Column(String)
    registration_count = Column(String)
    login_count = Column(String)
    email_registration_count = Column(String)
    device_registration_count = Column(String)
    consumer_token_generation_count = Column(String)
    mfa_token_generation_count = Column(String)
    defaultapp_email_registration_count = Column(String)
    altapp1_email_registration_count = Column(String)