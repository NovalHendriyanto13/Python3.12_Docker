...
async def master_flow():
    ...
    if etl_source == 'PDC':
        ...
        # Excluded for now (2026-09-09): no matching Mongo collection and/or Postgres
        # table exists yet for these - see audit findings. Re-enable once the real
        # source/target is confirmed.
        # from etl.BDE.advertisement_activities_etl import etl_advertisement_activities_flow
        # from etl.BDE.customer_tags_etl import etl_consumer_tags_flow
        # from etl.BDE.loyalty_activities_etl import etl_flow as etl_loyalty_activities_flow
        # from etl.BDE.loyalty_program_rewards_etl import etl_flow as etl_loyalty_program_rewards_flow
        # from etl.BDE.loyalty_programs_etl import etl_flow as etl_loyalty_programs_flow
        # from etl.PDC.stamp_program_transactions import etl_stamp_program_transactions_flow
        # from etl.BDE.tag_values_etl import etl_flow as etl_tag_values_flow
        # points_program_transactions: table doesn't exist yet and the source collection
        # is an unconfirmed guess - excluded until confirmed.
        # from etl.BDE.points_program_transactions_etl import etl_flow as etl_points_program_transactions_flow
        ...
        from etl.BDE.advertisements_etl import etl_advertisement_flow
        ...
        from etl.BDE.messages_etl import etl_flow as etl_messages_flow
        from etl.BDE.offer_activities_etl import etl_flow as etl_offer_activities_flow
        from etl.BDE.offers_etl import etl_offers_flow
        from etl.BDE.pushmessage_activities_etl import etl_flow as etl_pushmessage_activities_flow
        from etl.BDE.sale_details_etl import etl_flow as etl_sale_details_flow
        from etl.BDE.sales_headers_etl import etl_sales_headers_flow
        from etl.PDC.stamp_program_reward_transactions_etl import etl_stamp_program_reward_transactions_flow
        from etl.BDE.venues_etl import etl_flow as etl_venues_flow
    else:
        etl_advertisement_activities_flow = None
